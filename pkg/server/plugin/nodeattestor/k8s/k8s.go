...
func (p *K8sPlugin) Attest(stream nodeattestor.Attest_PluginStream) error {
	req, err := stream.Recv()
	if err != nil {
		return err
	}
	...
	resp := &nodeattestor.AttestResponse{
		Valid:        true,
		BaseSPIFFEID: k8s.SpiffeID(c.trustDomain, leaf),
...
func (p *K8sPlugin) getConfiguration() *configuration {
	p.m.Lock()
	defer p.m.Unlock()
	return p.c
}

func (p *K8sPlugin) setConfiguration(c *configuration) {
	p.m.Lock()
	defer p.m.Unlock()
	p.c = c
}
...
func newError(format string, args ...interface{}) error {
	return fmt.Errorf("k8s: "+format, args...)
}
...
func buildSelectors(leaf *x509.Certificate, chains [][]*x509.Certificate) []*common.Selector {
	...
	if leaf.Subject.CommonName != "" {
		selectors = append(selectors, &common.Selector{
			Type: "k8s", Value: "subject:cn:" + leaf.Subject.CommonName,
		})
	}
	...
	for _, chain := range chains {
		...
		for _, cert := range chain[1:] {
			...
			selectors = append(selectors, &common.Selector{
				Type: "k8s", Value: "ca:fingerprint:" + fp,
			})
		}
	}
