...
func (s *Suite) TestAttestSuccess() {
	...
	require.NoError(err)
	s.True(resp.Valid)
	require.Equal("spiffe://"+trustDomain+"/spire/agent/k8s/"+s.idDoc.Subject.CommonName, resp.BaseSPIFFEID)
...
func (s *Suite) TestAttestFailure() {
	...
	attestFails(makeData(&x509pop.AttestationData{Certificates: [][]byte{unauthCert.Raw}}),
		"k8s: certificate verification failed")

	// malformed challenge response
	challengeResponseFails("", "k8s: unable to unmarshal challenge response")

	// invalid response
	challengeResponseFails("{}", "k8s: challenge response verification failed")
}
